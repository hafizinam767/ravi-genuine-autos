          ...
          <SelectContent>
            ...
            {Object.entries(carModelsByMake).map(([make, models]) => (
              <React.Fragment key={make}>
                <SelectItem value={`__group_${make}`} disabled className="font-semibold text-amber-600">
                  {make}
                </SelectItem>
                {models.map((model) => (
  ...
  const activeFilterBadges = (
    <div className="flex flex-wrap gap-2">
      {filters.carModelId && (
        <Badge
          variant="secondary"
          className="gap-1 pr-1 bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-100"
        >
        ...
        <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
          <SheetTrigger asChild>
            <Button variant="outline" size="sm" className="gap-2">
              ...
              Filters
              {activeFilterCount > 0 && (
                <Badge className="bg-amber-500 text-white border-0 ml-1 px-1.5 py-0 text-xs">
                  {activeFilterCount}
                </Badge>
              )}
            </Button>
