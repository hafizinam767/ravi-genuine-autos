  ...
  const identifyPart = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(
          data.error ||
            'Identification failed. Please try again or contact us at 0320-0408917.'
        );
        return;
      }
      ...
      setError(
        'Network error. Please check your connection and try again, or contact us at 0320-0408917.'
      );
    } finally {
        ...
        {!imagePreview ? (
            ...
            <div
              ...
              className={`group relative cursor-pointer rounded-2xl border-2 border-dashed p-8 text-center transition-all duration-200 sm:p-12 ${
                isDragOver
                  ...
                  : 'border-border hover:border-amber-400 hover:bg-amber-50/50 dark:hover:bg-amber-950/10'
              }`}
            >
              <input
                ref={fileInputRef}
            ...
            <div className="mt-6">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {EXAMPLE_PARTS.map((ex) => (
                  <button
                    key={ex.label}
                    className="inline-flex items-center gap-1.5 rounded-full border border-border bg-background px-4 py-2 text-sm text-foreground transition-colors hover:border-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/20"
                  >
                    <span>{ex.emoji}</span>
                    {ex.label}
                  </button>
          ...
          >
            ...
            <Button
              ...
              className="w-full gap-2 bg-amber-500 py-6 text-base font-semibold text-white hover:bg-amber-600 disabled:opacity-60"
              size="lg"
            >
              {isIdentifying ? (
                <>
        ...
        {isIdentifying && (
          ...
          >
            <div className="relative">
              ...
              <Sparkles className="absolute left-1/2 top-1/2 h-6 w-6 -translate-x-1/2 -translate-y-1/2 text-amber-500" />
            </div>
              ...
              <CardContent className="space-y-4 pt-4">
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                  ...
                  <Button
                    variant="outline"
                    onClick={resetAll}
                    className="flex-1 gap-2 border-amber-300 text-amber-700 hover:bg-amber-50 dark:border-amber-700 dark:text-amber-400 dark:hover:bg-amber-950/20"
                    size="lg"
                  >
